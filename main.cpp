...
main(int ac, const char* av[])
{
    ...
    lfieg::CmdLineOptions opts {ac, av};
    ...
    lfieg::rpccalls::login_opt daemon_rpc_login {};
    ...
    if (!lfieg::get_blockchain_path(bc_path_opt, blockchain_path, nettype))
    {
    ...
    lfieg::MicroCore mcore;
    cryptonote::Blockchain* core_storage;

    // initialize mcore and core_storage
    if (!lfieg::init_blockchain(blockchain_path.string(),
    ...
    if (testnet && daemon_url == "127.0.0.1:25251")
        daemon_url = "127.0.0.1:22822";
    if (stagenet && daemon_url == "127.0.0.1:25251")
        daemon_url = "127.0.0.1:33833";
    ...
    if (enable_emission_monitor == true)
    {
        ...
        lfieg::CurrentBlockchainStatus::blockchain_path
                = blockchain_path;
        lfieg::CurrentBlockchainStatus::nettype
                = nettype;
        lfieg::CurrentBlockchainStatus::daemon_url
                = daemon_url;
        lfieg::CurrentBlockchainStatus::set_blockchain_variables(
                &mcore, core_storage);
        ...
        lfieg::CurrentBlockchainStatus::start_monitor_blockchain_thread();
    }


    lfieg::MempoolStatus::blockchain_path
            = blockchain_path;
    lfieg::MempoolStatus::nettype
            = nettype;
    lfieg::MempoolStatus::daemon_url
            = daemon_url;
    lfieg::MempoolStatus::login
            = daemon_rpc_login;
    lfieg::MempoolStatus::set_blockchain_variables(
            &mcore, core_storage);

    lfieg::MempoolStatus::network_info initial_info;
    strcpy(initial_info.block_size_limit_str, "0.0");
    strcpy(initial_info.block_size_median_str, "0.0");
    lfieg::MempoolStatus::current_network_info = initial_info;
    ...
    // by tx searching threads that are launched for each user independently,
    // when they log back or create new account.
    lfieg::MempoolStatus::mempool_refresh_time = mempool_refresh_time;
    lfieg::MempoolStatus::start_mempool_status_thread();

    // create instance of page class which
    // contains logic for the website
    lfieg::page lfiblocks(&mcore,
                          core_storage,
                          daemon_url,
                          nettype,
                          enable_pusher,
    ...
    CROW_ROUTE(app, "/")
    ([&]() {
        return mylfi::htmlresponse(lfiblocks.index2());
    });

    CROW_ROUTE(app, "/page/<uint>")
    ([&](size_t page_no) {
        return mylfi::htmlresponse(lfiblocks.index2(page_no));
    });

    CROW_ROUTE(app, "/block/<uint>")
    ([&](size_t block_height) {
        return mylfi::htmlresponse(lfiblocks.show_block(block_height));
    });
    
    CROW_ROUTE(app, "/randomx/<uint>")
    ([&](size_t block_height) {
        return mylfi::htmlresponse(lfiblocks.show_randomx(block_height));
    });

    CROW_ROUTE(app, "/block/<string>")
    ([&](string block_hash) {
        return mylfi::htmlresponse(
                lfiblocks.show_block(remove_bad_chars(block_hash)));
    });

    CROW_ROUTE(app, "/tx/<string>")
    ([&](string tx_hash) {
        return mylfi::htmlresponse(
                lfiblocks.show_tx(remove_bad_chars(tx_hash)));
    });
    if (enable_autorefresh_option)
    {
        ...
        ([&](string tx_hash) {
            ...
            return mylfi::htmlresponse(
                lfiblocks.show_tx(remove_bad_chars(tx_hash), with_ring_signatures, refresh_page));
        });
    }
    ...
    if (enable_as_hex)
    {
        ...
        ([&](string tx_hash) {
            return crow::response(
                    lfiblocks.show_tx_hex(remove_bad_chars(tx_hash)));
        });

        CROW_ROUTE(app, "/ringmembershex/<string>")
        ([&](string tx_hash) {
            return crow::response(
                    lfiblocks.show_ringmembers_hex(remove_bad_chars(tx_hash)));
        });

        CROW_ROUTE(app, "/blockhex/<uint>")
        ([&](size_t block_height) {
            return crow::response(
                    lfiblocks.show_block_hex(block_height, false));
        });

        CROW_ROUTE(app, "/blockhexcomplete/<uint>")
        ([&](size_t block_height) {
            return crow::response(
                    lfiblocks.show_block_hex(block_height, true));
        });

//        CROW_ROUTE(app, "/ringmemberstxhex/<string>")
//        ([&](string tx_hash) {
//            return crow::response(
//              lfiblocks.show_ringmemberstx_hex(remove_bad_chars(tx_hash)));
//        });

        CROW_ROUTE(app, "/ringmemberstxhex/<string>")
        ([&](string tx_hash) {
            return mylfi::jsonresponse {
                lfiblocks.show_ringmemberstx_jsonhex(
    ...
    if (enable_autorefresh_option)
    {
        ...
        ([&](string tx_hash, uint16_t with_ring_signature) {
            bool refresh_page {true};
            return mylfi::htmlresponse(
                lfiblocks.show_tx(remove_bad_chars(tx_hash), with_ring_signature, refresh_page));
        });
    }

    CROW_ROUTE(app, "/myoutputs").methods("POST"_method)
    ([&](const crow::request& req) -> mylfi::htmlresponse
     {
        ...
        return mylfi::htmlresponse(std::move(response));
    });
    ...
    if (enable_pusher)
    {
        ...
        ([&]() {
            return mylfi::htmlresponse(lfiblocks.show_rawtx());
        });
        ...
        ([&](const crow::request& req) -> mylfi::htmlresponse
         {
            ...
            if (action == "check")
                return mylfi::htmlresponse(
                        lfiblocks.show_checkrawtx(raw_tx_data, action));
            else if (action == "push")
                return mylfi::htmlresponse(
                        lfiblocks.show_pushrawtx(raw_tx_data, action));
    ...
    if (enable_key_image_checker)
    {
        ...
        ([&]() {
            return mylfi::htmlresponse(lfiblocks.show_rawkeyimgs());
        });
        ...
        ([&](const crow::request& req) -> mylfi::htmlresponse
         {
            ...
            return mylfi::htmlresponse(
                    lfiblocks.show_checkrawkeyimgs(raw_data, viewkey));
        });
    }
    ...
    if (enable_output_key_checker)
    {
        ...
        ([&]() {
            return mylfi::htmlresponse(lfiblocks.show_rawoutputkeys());
        });
        ...
        ([&](const crow::request& req) -> mylfi::htmlresponse
         {
            ...
            return mylfi::htmlresponse(
                    lfiblocks.show_checkcheckrawoutput(raw_data, viewkey));
        });
    }
    ...
    ([&]() {
        return mylfi::htmlresponse(lfiblocks.mempool(true));
    });
...
//    });
    ...
    if (enable_json_api)
    {
        ...
        ([&](string tx_hash) {
            ...
            mylfi::jsonresponse r{lfiblocks.json_transaction(remove_bad_chars(tx_hash))};
        ...
        ([&](string tx_hash) {
            ...
            mylfi::jsonresponse r{lfiblocks.json_rawtransaction(remove_bad_chars(tx_hash))};
        ...
        ([&](string tx_hash) {
            ...
            mylfi::jsonresponse r{lfiblocks.json_detailedtransaction(remove_bad_chars(tx_hash))};
        ...
        ([&](string block_no_or_hash) {
            ...
            mylfi::jsonresponse r{lfiblocks.json_block(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](string block_no_or_hash) {
            ...
            mylfi::jsonresponse r{lfiblocks.json_rawblock(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](const crow::request &req) {
            ...
            mylfi::jsonresponse r{lfiblocks.json_transactions(
        ...
        ([&](const crow::request &req) {
            ...
            mylfi::jsonresponse r{lfiblocks.json_mempool(
                    remove_bad_chars(page), remove_bad_chars(limit))};
        ...
        ([&](string search_value) {
            ...
            mylfi::jsonresponse r{lfiblocks.json_search(remove_bad_chars(search_value))};
        ...
        ([&]() {
            ...
            mylfi::jsonresponse r{lfiblocks.json_networkinfo()};
        ...
        ([&]() {
            ...
            mylfi::jsonresponse r{lfiblocks.json_emission()};
        ...
        ([&](const crow::request &req) {
            ...
            mylfi::jsonresponse r{lfiblocks.json_outputs(
                    remove_bad_chars(tx_hash),
        ...
        ([&](const crow::request &req) {
            ...
            mylfi::jsonresponse r{lfiblocks.json_outputsblocks(
        ...
        ([&]() {
            ...
            mylfi::jsonresponse r{lfiblocks.json_version()};
    ...
    if (enable_autorefresh_option)
    {
        ...
        ([&]() {
            uint64_t page_no {0};
            bool refresh_page {true};
            return mylfi::htmlresponse(lfiblocks.index2(page_no, refresh_page));
        });
    }
    ...
    if (enable_emission_monitor == true)
    {
        ...
        lfieg::CurrentBlockchainStatus::m_thread.interrupt();
        lfieg::CurrentBlockchainStatus::m_thread.join();
    ...
    lfieg::MempoolStatus::m_thread.interrupt();
    lfieg::MempoolStatus::m_thread.join();
