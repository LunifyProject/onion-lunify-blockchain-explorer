...
/**
* Parse Lunify address in a string form into
...
/**
* Return string representation of Lunify address
*/
string
...
{
    // default path to Lunify folder
    // on linux this is /home/<username>/.Lunify
    string default_LUNIFY_DIR = tools::get_default_data_dir();

    if (nettype == cryptonote::network_type::TESTNET)
        default_LUNIFY_DIR += "/testnet";
    if (nettype == cryptonote::network_type::STAGENET)
        default_LUNIFY_DIR += "/stagenet";


    // the default folder of the lmdb blockchain database
    // is therefore as follows
    return default_LUNIFY_DIR + string("/lmdb");
}
...
{
    // the default folder of the lmdb blockchain database
    string default_lmdb_dir   = lfieg::get_default_lmdb_folder(nettype);
    ...
    blockchain_path = lfieg::remove_trailing_path_separator(blockchain_path);
...
{
    pair<uint64_t, uint64_t> sum_lfi {0, 0};
    ...
    {
        sum_lfi.first += vout["amount"].get<uint64_t>();
        ++sum_lfi.second;
    }
...
{
    pair<uint64_t, uint64_t> sum_lfi {0ULL, 0ULL};
        ...
        vector<txin_to_key>& input_key_imgs)
{
    ...
    uint64_t lfi_outputs       {0};
    uint64_t lfi_inputs        {0};
    uint64_t mixin_no          {0};
    ...
    for (size_t i = 0; i < input_no; ++i)
    {
        ...
        lfi_inputs += tx_in_to_key.amount;
    ...
    return {lfi_outputs, lfi_inputs, mixin_no, num_nonrct_inputs};
};
...
summary_of_in_out_rct(const json& _json)
{
    ...
    for (const json& vout: _json["vout"])
    {
        lfi_outputs += vout["amount"].get<uint64_t>();
    }
    ...
    return {lfi_outputs, lfi_inputs, no_outputs, no_inputs, mixin_no, num_nonrct_inputs};
};
...
sum_money_in_inputs(const string& json_str)
{
    ...
    for (json& vin: j["vin"])
    {
        sum_lfi.first += vin["key"]["amount"].get<uint64_t>();
        ++sum_lfi.second;
    }
...
{
    array<uint64_t, 2> sum_lfi;

    sum_lfi[0] = sum_money_in_inputs(tx);
    sum_lfi[1] = sum_money_in_outputs(tx);
               ...
               const public_key& public_spend_key)
{
    ...
    {
        cerr << "Cant get dervied key" << endl;
