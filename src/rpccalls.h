...
#ifndef CROWLFI_RPCCALLS_H
#define CROWLFI_RPCCALLS_H
...
{
// declare struct in Lunify's cryptonote namespace.
// Lunify should provide definition for this,
...
public:
    ...
    rpccalls(string _daemon_url = "http:://127.0.0.1:25252",
             login_opt _login = login_opt {},
             uint64_t _timeout = 200000);

    bool
    connect_to_lunify_daemon();
    ...
    get_alt_blocks(vector<string>& alt_blocks_hashes)
    {
        ...
        if (r)
        {
            ...
            if (!err.empty())
            {
                cerr << "Error connecting to Lunify daemon due to "
                     << err << endl;
                return false;
            }
        }
        ...
        {
            cerr << "Error connecting to Lunify daemon at "
                 << daemon_url << endl;
            return false;
        }
