...
#ifndef CROWLFI_PAGE_H
#define CROWLFI_PAGE_H



#include "mstch/mstch.hpp"

#include "lunify_headers.h"
#include "randomx.h"
#include "common.hpp"
...
// forked version of the rx_slow_hash from Lunify
...
// from Lunify to use me_rx_slow_hash
bool
          ...
          mainnet_url {_mainnet_url}
{
    ...
    template_file["css_styles"]      = lfieg::read(TMPL_CSS_STYLES);
    template_file["header"]          = lfieg::read(TMPL_HEADER);
    template_file["footer"]          = get_footer();
    template_file["index2"]          = get_full_page(lfieg::read(TMPL_INDEX2));
    template_file["mempool"]         = lfieg::read(TMPL_MEMPOOL);
    template_file["altblocks"]       = get_full_page(lfieg::read(TMPL_ALTBLOCKS));
    template_file["mempool_error"]   = lfieg::read(TMPL_MEMPOOL_ERROR);
    template_file["mempool_full"]    = get_full_page(template_file["mempool"]);
    template_file["block"]           = get_full_page(lfieg::read(TMPL_BLOCK));
    template_file["randomx"]         = get_full_page(lfieg::read(TMPL_RANDOMX));
    template_file["tx"]              = get_full_page(lfieg::read(TMPL_TX));
    template_file["my_outputs"]      = get_full_page(lfieg::read(TMPL_MY_OUTPUTS));
    template_file["rawtx"]           = get_full_page(lfieg::read(TMPL_MY_RAWTX));
    template_file["checkrawtx"]      = get_full_page(lfieg::read(TMPL_MY_CHECKRAWTX));
    template_file["pushrawtx"]       = get_full_page(lfieg::read(TMPL_MY_PUSHRAWTX));
    template_file["rawkeyimgs"]      = get_full_page(lfieg::read(TMPL_MY_RAWKEYIMGS));
    template_file["rawoutputkeys"]   = get_full_page(lfieg::read(TMPL_MY_RAWOUTPUTKEYS));
    template_file["checkrawkeyimgs"] = get_full_page(lfieg::read(TMPL_MY_CHECKRAWKEYIMGS));
    template_file["checkoutputkeys"] = get_full_page(lfieg::read(TMPL_MY_CHECKRAWOUTPUTKEYS));
    template_file["address"]         = get_full_page(lfieg::read(TMPL_ADDRESS));
    template_file["search_results"]  = get_full_page(lfieg::read(TMPL_SEARCH_RESULTS));
    template_file["tx_details"]      = lfieg::read(string(TMPL_PARIALS_DIR) + "/tx_details.html");
    template_file["tx_table_header"] = lfieg::read(string(TMPL_PARIALS_DIR) + "/tx_table_header.html");
    template_file["tx_table_row"]    = lfieg::read(string(TMPL_PARIALS_DIR) + "/tx_table_row.html");
}
...
index2(uint64_t page_no = 0, bool refresh_page = false)
{
    ...
    mstch::map context {
            ...
            {"height"                   , height},
            {"server_timestamp"         , lfieg::timestamp_to_str_gm(local_copy_server_timestamp)},
    ...
    //double blk_size_median = lfieg::calc_median(blk_sizes.begin(), blk_sizes.end());
...
show_block(uint64_t _blk_height)
{
    ...
    string blk_timestamp = lfieg::timestamp_to_str_gm(blk.timestamp);
    ...
    for (size_t i = 0; i < blk.tx_hashes.size(); ++i)
    {
        ...
        //string mixin_times_scale = lfieg::timestamps_time_scale(mixin_timestamps,
        //                                                        server_timestamp);
...
show_block(string _blk_hash)
{
    ...
    if (!lfieg::parse_str_secret_key(_blk_hash, blk_hash))
    {
...
show_tx(string tx_hash_str, uint16_t with_ring_signatures = 0, bool refresh_page = false)
{
    ...
    if (!lfieg::parse_str_secret_key(tx_hash_str, tx_hash))
    {
    ...
    if (!mcore->get_tx(tx_hash, tx))
    {
        ...
        if (!found_txs.empty())
        {
            ...
            blk_timestamp = lfieg::timestamp_to_str_gm(tx_recieve_timestamp);
...
show_ringmemberstx_jsonhex(string const& tx_hash_str)
{
    ...
    // private Lunify networks, so we can easly put these
                ...
                bool tx_prove = false)
{
    ...
    if (!lfieg::parse_str_secret_key(viewkey_str, multiple_tx_secret_keys))
    {
...
//        if (!lfieg::parse_str_secret_key(spend_key_str, prv_spend_key))
//        {
...
//        }
    ...
    if (tx_blk_found)
    {
        ...
        blk_timestamp = lfieg::timestamp_to_str_gm(blk.timestamp);
    ...
    string server_time_str = lfieg::timestamp_to_str_gm(server_timestamp, "%F");
...
//                cout << derived_view_tag << endl;
        ...
        for (const txin_to_key& in_key: input_key_imgs)
        {
            ...
            for (const uint64_t& abs_offset: absolute_offsets)
            {
                ...
                public_key mixin_tx_pub_key
                        = lfieg::get_tx_pub_key_from_received_outs(mixin_tx);
                ...
                output_pub_keys = lfieg::get_ouputs_tuple(mixin_tx);
    ...
    //
    //                        if (!lfieg::generate_key_image(derivation,
...
show_checkrawtx(string raw_tx_data, string action)
{
    ...
    string data_prefix = lfieg::make_printable(decoded_raw_tx_data.substr(0, magiclen));
    ...
    if (unsigned_tx_given)
    {
        ...
        if (r)
        {
            ...
            for (const ::tools::wallet2::tx_construction_data& tx_cd: exported_txs.txes)
            {
                ...
                tx_cd_data["min_mix_time"]     = lfieg::timestamp_to_str_gm(min_mix_timestamp);
                tx_cd_data["max_mix_time"]     = lfieg::timestamp_to_str_gm(max_mix_timestamp);
...
show_pushrawtx(string raw_tx_data, string action)
{
    ...
    // if failed, treat raw_tx_data as base64 encoding of signed_lunify_tx
    else
    {
        string decoded_raw_tx_data = epee::string_encoding::base64_decode(raw_tx_data);
...
show_checkrawkeyimgs(string raw_data, string viewkey_str)
{
    ...
    if (!lfieg::parse_str_secret_key(viewkey_str, prv_view_key))
    {
    ...
    string data_prefix = lfieg::make_printable(decoded_raw_data.substr(0, magiclen));
    ...
    decoded_raw_data = lfieg::decrypt(
...
show_checkcheckrawoutput(string raw_data, string viewkey_str)
{
    ...
    for (const tools::wallet2::transfer_details& td: outputs)
    {
        ...
        if (td.is_rct())
        {
            ...
            public_key tx_pub_key = lfieg::get_tx_pub_key_from_received_outs(tx);
...
search(string search_text)
{
    ...
    // check if Lunify address is given based on its length
    ...
    if (search_str_length == 95)
    {
        ...
        if (!lfieg::parse_str_address(search_text, address_info, nettype_addr))
        {
    ...
    // check if integrated Lunify address is given based on its length
...
show_address_details(const address_parse_info& address_info, cryptonote::network_type nettype = cryptonote::network_type::MAINNET)
{
    ...
    string address_str      = lfieg::print_address(address_info, nettype);
    ...
    for (const pair<string, vector<string>>& found_txs: all_possible_tx_hashes)
    {
        ...
        if (!found_txs.second.empty())
        {
            ...
            for (const string& tx_hash: found_txs.second)
            {
                ...
                txd_map.insert({"timestamp", lfieg::timestamp_to_str_gm(blk_timestamp)});
...
json_transaction(string tx_hash_str)
{
    ...
    string blk_timestamp_utc = lfieg::timestamp_to_str_gm(tx_timestamp);
...
json_block(string block_no_or_hash)
{
    ...
    else if (block_no_or_hash.length() == 64)
    {
        ...
        if (!lfieg::parse_str_secret_key(block_no_or_hash, blk_hash))
        {
    ...
    j_data = json {
            ...
            {"timestamp_utc" , lfieg::timestamp_to_str_gm(blk.timestamp)},
            {"block_height"  , block_height},
            {"size"          , blk_size},
            {"txs"           , j_txs},
             ...
             bool tx_prove = false)
{
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Lunify address not provided";
        return j_response;
    }
    ...
    if (!lfieg::parse_str_address(address_str,  address_info, nettype))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant parse Lunify address: " + address_str;
        return j_response;
...
json_networkinfo()
{
    ...
    if (!get_lunify_network_info(j_info))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant get Lunify network info";
    //    return j_response;
    }
...
json_version()
{
    ...
    j_data = json {
            ...
            {"lunify_version_full" , string {lunify_VERSION_FULL}},
...
construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
{
    ...
    double payed_for_kB = LFI_AMOUNT(txd.fee) / tx_size;
        ...
        for (const uint64_t& i: absolute_offsets)
        {
            ...
            if (detailed_view)
            {
                ...
                mixins.push_back(mstch::map {
                        ...
                        {"mix_timestamp",  lfieg::timestamp_to_str_gm(blk.timestamp)},
    ...
    if (detailed_view)
    {
        ...
        context["min_mix_time"]     = lfieg::timestamp_to_str_gm(min_mix_timestamp);
        context["max_mix_time"]     = lfieg::timestamp_to_str_gm(max_mix_timestamp);
...
)
{
    ...
    {
        // get mixins in time scale for visual representation
        pair<string, double> mixin_times_scale = lfieg::timestamps_time_scale(
                mixn_timestamps,
                max_mix_timestamp,
                170,
                min_mix_timestamp);
               ...
               uint64_t bc_height = 0)
{
    ...
    txd.pk = lfieg::get_tx_pub_key_from_received_outs(tx);
...
bool
get_lunify_network_info(json& j_info)
{
...
get_footer()
{
    ...
    string footer_html = mstch::render(lfieg::read(TMPL_FOOTER), footer_context);
