...
Currently available Lunify blockchain explorers have several limitations which are of
special importance to privacy-oriented users:
 ...
 - do not support Lunify testnet nor stagenet networks,
 - have limited JSON API.


In this example, these limitations are addressed by development of
an Onion Lunify Blockchain Explorer. The example not only shows how to use
Lunify C++ libraries, but also demonstrates how to use:
...
## Onion Lunify Blockchain Explorer features

The key features of the Onion Lunify Blockchain Explorer are:
 ...
 - showing public components of Lunify addresses,
 - decoding which outputs and mixins belong to the given Lunify address and viewkey,
 - can prove that you send Lunify to someone,
 - detailed information about ring members, such as their age, timescale, and ring sizes,
 - showing number of amount output indices,
 - support Lunify testnet and stagnet networks,
...
Once the Lunify compiles, the explorer can be downloaded and compiled
as follows:

```bash
# go to home folder if still in ~/Lunify
cd ~

# download the source code 
git clone https://github.com/LunifyProject/onion-lunify-blockchain-explorer.git

# enter the downloaded sourced code folder
cd onion-lunify-blockchain-explorer
...
against latest release (`release-v0.17`) branch of Lunify:
...
To run it, mount the lunify blockchain onto the container as volume.
...
services:
  lunifyd:
    image: sethsimmons/simple-lunifyd:latest
...
```bash
git clone https://github.com/lunifyexamples/onion-lunify-blockchain-explorer.git
...
xlablocks, Onion Lunify Blockchain Explorer:
  ...
  --enable-emission-monitor [=arg(=1)] (=0)
                                        enable Lunify total emission monitoring
                                        thread
...
Obtaining current Lunify emission amount is not straight forward. Thus, by default it is
...
Every 10000 blocks, the thread will save current emission in a file, by default,
 in `~/.Lunify/lmdb/emission_amount.txt`. For testnet or stagenet networks,
 it is `~/.Lunify/testnet/lmdb/emission_amount.txt` or `~/.Lunify/stagenet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the
 explorer restarts, the thread will first check if `~/.Lunify/lmdb/emission_amount.txt`
...
```
Lunify emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using Lunify daemon's  `print_coinbase_tx_sum` command.
...
```bash
# we use here official Lunify project's donation address as an example
...
```json
{
  "data": {
    ...
    "git_branch_name": "update_to_current_lunify",
    "last_git_commit_date": "2017-07-25",
    "last_git_commit_hash": "a549f25",
    "lunify_version_full": "0.10.3.1-ab594cfe"
  },
  "status": "success"
}
```
...
Return raw json block data, as represented in Lunify.
...
Return raw json tx data, as represented in Lunify.
...
## Other lunify examples

Other examples can be found on  [github](https://github.com/lunifyexamples?tab=repositories).
